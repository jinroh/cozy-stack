...
func GenerateSecureLinkSecret(key []byte, doc *FileDoc, sessionID string) string {
	...
	mac, err := crypto.EncodeAuthMessage(downloadLinkMac, key, doc.MD5Sum,
		additionalData)
	if err != nil {
		return ""
	}
...
func VerifySecureLinkSecret(key []byte, secret, fileID, sessionID string) bool {
	...
	additionalData := []byte(fileID + sessionID)
	_, err := crypto.DecodeAuthMessage(downloadLinkMac, key, []byte(secret),
		additionalData)
	return err == nil
}
